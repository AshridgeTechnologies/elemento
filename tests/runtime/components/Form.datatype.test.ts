...
import {componentJSONAsync, getCallArg, testAppInterface, wait, wrappedTestElement} from '../../testutil/testHelpers'
...
class PlainFormState extends DataTypeFormState {
}

function PlainForm(props: {path: string, initialValue: object, dataType: RecordType, keyAction: KeyboardEventHandler}) {
    const pathWith = (name: string) => props.path + '.' + name

    const {initialValue, dataType} = props
    const _state = useComponentState(props.path, PlainFormState, {initialValue, dataType})
...
function TestForm(props: {path: string, initialValue: object, dataType: RecordType, keyAction: KeyboardEventHandler}) {
    const pathWith = (name: string) => props.path + '.' + name

    const {initialValue, dataType} = props
    const _state = useComponentState(props.path, TestFormState, {initialValue, dataType})
...
test('Form element produces output with no children', async () => {
...
test('Form element produces output with one child', async () => {
...
test('State class has correct properties and functions', () => {
    ...
    state.Reset()
    expect(appInterface.latest()._stateForTest).toStrictEqual({errorsShown: false})
})
...
test('State has expected values', async () => {
    ...
    await actWait(50)
    expect(testFormStateAt('form1').value).toStrictEqual({Description: 'Big', BoxSize: 17, Count: null})
    expect(testFormStateAt('form1.Description').value).toBe('Big')
...
test('State has own errors', async () => {
    const {enter}  = testContainer(form('app.page5.form1', {
        dataType: recordType,
        initialValue: {Description: 'Big', BoxSize: 5}
    }), 'testContainer4')
    await wait()
