...
test('Calculation shows value from the state supplied', () => {
    const {el} = testContainer(calculation('app.page1.widget1', {calculation:'Hello!', show: true}))
...
test('Calculation asynchronously checks and triggers when action from UI element when value true', async () => {
    const action = vi.fn()
    testContainer(calculation('app.page1.widget1', {calculation: true, whenTrueAction: action}))
...
test('Calculation does not triggers when action from UI element when value false', () => {
    const action = vi.fn()
    testContainer(calculation('app.page1.widget1', {calculation: null, whenTrueAction: action}))
    expect(action).not.toHaveBeenCalled()
})

test('State class has correct properties and functions', () => {
    const state = new Calculation.State({calculation: {a: 10, b: 'bee'}})
    expect(state.value).toStrictEqual({a: 10, b: 'bee'})
})

test('State class does nothing on trigger if no when true action', () => {
    const state = new Calculation.State({calculation: true})
...
test('State class does nothing on trigger if false and previous value missing but updates previous value', () => {
    const action = vi.fn()
    const state = new Calculation.State({calculation: false, whenTrueAction: action})
    const appInterface = testAppInterface('testPath', state)
    state.checkTriggered()
    expect(action).not.toHaveBeenCalled()
    expect(appInterface.latest()._stateForTest.previousValueTruthy).toBe(false)
})

test('State class does nothing on trigger if false and does not update previous value if false', () => {
    const action = vi.fn()
    const state = new Calculation.State({calculation: false, whenTrueAction: action}).withState({previousValueTruthy: false})
    const appInterface = testAppInterface('testPath', state)
    state.checkTriggered()
    expect(action).not.toHaveBeenCalled()
    expect(appInterface.update).not.toHaveBeenCalled()
...
test('State class runs action on trigger if truthy and previous value false and updates previous value', () => {
    const action = vi.fn()
    const state = new Calculation.State({calculation: 1, whenTrueAction: action}).withState({previousValueTruthy: false})
    const appInterface = testAppInterface('testPath', state)
    state.checkTriggered()
    expect(action).toHaveBeenCalled()
    expect(appInterface.latest()._stateForTest.previousValueTruthy).toBe(true)
})

test('State class does not run action on trigger if truthy and previous value truthy and does not update previous value', () => {
    const action = vi.fn()
    const state = new Calculation.State({calculation: 2, whenTrueAction: action}).withState({previousValueTruthy: true})
    const appInterface = testAppInterface('testPath', state)
    state.checkTriggered()
    expect(action).not.toHaveBeenCalled()
    expect(appInterface.update).not.toHaveBeenCalled()
...
test('State class does not run action on trigger if falsy and previous value truthy but updates previous value', () => {
    const action = vi.fn()
    const state = new Calculation.State({calculation: null, whenTrueAction: action}).withState({previousValueTruthy: true})
    const appInterface = testAppInterface('testPath', state)
    state.checkTriggered()
    expect(action).not.toHaveBeenCalled()
    expect(appInterface.latest()._stateForTest.previousValueTruthy).toBe(false)
...
test('State class gives correct value when its value is a value object', () => {
    const state = new Calculation.State({calculation: valueObj('car')})
    expect(state.value).toBe('car')
})

test('string conversion uses the value toString', () => {
    const calculation = [1, 2, 3]
    const state = new Calculation.State({calculation})
