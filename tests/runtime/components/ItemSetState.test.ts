...
import {createStateFn, valueObj} from '../../testutil/testHelpers'
...
test('knows if item is selected', () => {
    const state = createState({items: [item1, item2], selectedItems: item1})
...
test('knows if item with id is selected', () => {
    const state = createState({items: [idItem1, idItem2], selectedItems: idItem1})
...
test('Can Set and Reset selected items, selectAction not called', () => {
    const state = createState({items: [item1, item2], selectedItems: item1, selectAction})
    expect(state.selectedItems).toStrictEqual([item1])
...
test('Can Select to add to selected items and Reset', () => {
    const state = createState({items: [item1, item2, item3, item4], selectedItems: item1, selectAction})
    expect(state.selectedItems).toStrictEqual([item1])

    state.Select(item4)
    expect(state.selectedItems).toStrictEqual([item1, item4])

    state.Select(valueObj([item3, item2]))
    expect(state.selectedItems).toStrictEqual([item1, item4, item3, item2])
...
test('Duplicates ignored in original selected items and Select items', () => {
    const state = createState({items: [item1, item2, item3, item4], selectedItems: [item1, item1]})
    expect(state.selectedItems).toStrictEqual([item1])

    state.Select(item4)
    expect(state.selectedItems).toStrictEqual([item1, item4])

    state.Select([item2, item4, item2])
    expect(state.selectedItems).toStrictEqual([item1, item4, item2])
})
...
test('Has new selected item after Set with item id', () => {
    const state = createState({items: [idItem3, idItem1, idItem2], selectedItems: idItem1})
    expect(state.selectedItem).toStrictEqual(idItem1)
...
test('Has new single selected item after onSelect with item id, addRemove or fromLast and selectable single', () => {
    const state = createState({items: [idItem1, idItem2], selectedItems: idItem1, selectAction, selectable: 'single'})
    expect(state.selectedItem).toStrictEqual(idItem1)
    ...
    state.onSelect(idItem1.id, 0, 'fromLast')
    expect(state.selectedItems).toStrictEqual([idItem1])
    expect(state.selectedItemIds).toStrictEqual([idItem1.id])
...
test('State class adds new single selected item after onSelect and addRemove if selectable is multiple', () => {
    const state = createState({items: [item1, item2, item3, item4], selectedItems: item1, selectAction, selectable: 'multiple'})
    expect(state.selectedItem).toStrictEqual(item1)
    ...
    state.onSelect(3, 3, 'addRemove')
    expect(state.selectedItems).toStrictEqual([item1, item2, item4])
    expect(state.selectedItemIds).toStrictEqual([0, 1, 3])
    expect(state.selectedItem).toStrictEqual(item4)
...
test('State class removes new single selected item in onSelect and addRemove if select is multiple and already selected in properties', () => {
    const state = createState({items: [item1, item2, item3, item4], selectedItems: [item1, item3], selectAction, selectable: 'multiple'})
    ...
    state.onSelect(0, 0, 'addRemove')
    expect(state.selectedItems).toStrictEqual([])
    expect(state.selectedItemIds).toStrictEqual([])
    expect(state.selectedItem).toStrictEqual(null)
    expect(selectAction).toHaveBeenCalledWith(null, null, null)
})

test('State class always adds new single selected item after onSelect if selectable is multipleAuto', () => {
    const state = createState({items: [item1, item2, item3, item4], selectedItems: item1, selectAction, selectable: 'multipleAuto'})
    expect(state.selectedItem).toStrictEqual(item1)
...
test('State class always removes new single selected item in onSelect if selectable is multipleAuto and already selected in properties', () => {
    const state = createState({items: [item1, item2, item3, item4], selectedItems: [item1, item3], selectAction, selectable: 'multipleAuto'})
    expect(state.selectedItems).toStrictEqual([item1, item3])
    expect(state.selectedItemIds).toStrictEqual([0, 2])
...
test('State class sets new block of selected items in onSelect with fromLast if select is multiple', () => {
    const state = createState({items: [item1, item2, item3, item4], selectedItems: [item1, item2], selectAction, selectable: 'multiple'})
    expect(state.selectedItems).toStrictEqual([item1, item2])

    state.onSelect(3, 3, 'fromLast')
    expect(state.selectedItems).toStrictEqual([item2, item3, item4])
    expect(state.selectedItemIds).toStrictEqual([1, 2, 3])
    expect(state.selectedItem).toStrictEqual(item4)
...
test('State class sets new block of selected items in onSelect with fromLast and item before last selected', () => {
    const state = createState({items: [idItem1, idItem2, idItem3, idItem4], selectedItems: [idItem3, idItem4], selectAction, selectable: 'multiple'})
    expect(state.selectedItems).toStrictEqual([idItem3, idItem4])

    state.onSelect(idItem2.id, 1, 'fromLast')
    expect(state.selectedItems).toStrictEqual([idItem4, idItem3, idItem2])
    expect(state.selectedItemIds).toStrictEqual([idItem4.id, idItem3.id, idItem2.id])
    expect(state.selectedItem).toStrictEqual(idItem2)
...
test('State class sets new block of selected items from first item in onSelect with fromLast if select is multiple and none selected', () => {
    const state = createState({items: [item1, item2, item3, item4], selectAction, selectable: 'multiple'})
    expect(state.selectedItems).toStrictEqual([])

    state.onSelect(2, 2, 'fromLast')
    expect(state.selectedItems).toStrictEqual([item1, item2, item3])
    expect(state.selectedItemIds).toStrictEqual([0, 1, 2])
    expect(state.selectedItem).toStrictEqual(item3)
...
test('Has no new selected item after onSelect if selectable is none, but selectAction is called', () => {
    const state = createState({items: [item1, item2], selectedItems: item1, selectAction, selectable: 'none'})
    expect(state.selectedItem).toStrictEqual(item1)
