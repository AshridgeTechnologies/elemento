...
test('DateInput element produces output with properties supplied',
    htmlSnapshot(dateInput('app.page1.start', {initialValue: new Date('2021-11-12'), label: 'Starting Date', styles: {color: 'red'}}))
)

test('DateInput element produces readonly output with properties supplied',
    htmlSnapshot(dateInput('app.page1.start', {initialValue: new Date('2021-11-12'), label: 'Starting Date', readOnly: true}))
)

test('DateInput element produces output with DataType supplied',
    htmlSnapshot(dateInput('app.page1.start1', {initialValue: new Date('2021-11-12'), dataType: dateType1, label: 'Starting Date'}))
)
...
test('DateInput element produces output with description info', () => {
    ...
    const {container} = render(dateInput('app.page1.description', {initialValue: new Date('2021-11-12'), dataType: dateType, label: 'Starting Date'}))
    expect(container.innerHTML).toMatchSnapshot()
})

test('DateInput shows value from the state supplied', () => {
    const {el} = testContainer(dateInput('app.page1.widget1', {initialValue: new Date('2021-11-12')}))
...
test('DateInput element produces output with properties supplied as state objects', () => {
    const {el} = testContainer(dateInput('app.page1.widget2', {
        initialValue: new Date('2021-11-12'),
        label: new TextInputState({initialValue: 'Item Date'})
    }))
...
test('DateInput shows empty value when state value is set to undefined', () => {
    const {el} = testContainer(dateInput('app.page1.widget3', {initialValue: undefined}))
    expect(el`app.page1.widget3`.value).toBe('')
})

test('DateInput shows initial value when state value  exists', () => {
    const {el} = testContainer(dateInput('app.page1.widget4', {initialValue: new Date('2021-11-12')}))
...
test('DateInput stores updated values in the app store section for its path', async () => {
    const {enter}  = testContainer(dateInput('app.page1.singleDate', {initialValue: new Date('2021-11-12')}))
...
test('DateInput stores null value in the app store when cleared', async () => {
    const {el, user} = testContainer(dateInput('app.page1.termDate', {initialValue: new Date('2021-11-12')}))
...
test('State class has correct properties', () => {
    ...
    state.Reset()
    expect(state.value).toStrictEqual(new Date('2021-11-12'))
    expect(state.dataValue).toStrictEqual(new Date('2021-11-12'))
...
test('DateInput stores errors shown when blurred and ShowErrors or Reset called', async () => {
    const {el, user, domContainer} = testContainer(dateInput('app.page1.startDate', {initialValue: new Date('2020-05-06'), dataType: dateType1}))
...
test('DateInput shows errors if errorShown is true', async () => {
    ...
    const {el} = testContainer(dateInput(inputId, {initialValue: new Date('2019-05-06'), dataType: dateType1, label: 'The Input'}))
