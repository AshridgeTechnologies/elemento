...
function Child(props: object) {
    stateInChild = useComponentState('app.parent.child', Thing, {a: 10, b: 'Bee'})
    return createElement('div', props)
}

function Parent(props: object) {
    stateInParent = useComponentState('app.parent.child')
...
test('Parent can get state set by child with the same class', async () => {
    ...
    expect(stateInParent._original).toBe(stateInChild._original)
})
