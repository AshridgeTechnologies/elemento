...
export type Status = 'waiting' | 'updating' | 'complete'
/**
...
export default class ThrottledCombinedFileWriter implements FileWriter {
    ...
    constructor(private readonly fileWriter: CombinedFileWriter, private readonly interval: number, private readonly onStatusChange: (status: Status) => void = noop) {
    }
    ...
    private writePendingFiles() {
        ...
        return this.fileWriter.writeFiles(filesToWrite).then( ()=> this.updateStatus('complete') )
    }
