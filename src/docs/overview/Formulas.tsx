...
export default () =>
    <Section id='formulas'>
        ...
        <Para>
            The names in these examples, like <code>Length</code> or <code>ItemTotal</code> are the names of <HelpLink
            id='elements'>elements</HelpLink> in the app.
            The app uses the current value of the element in the formula.
        </Para>
        <Para>So if you had a <HelpLink id='textInput'/> element where someone could enter their name, and you gave it the name "First Name",
            you can use <code>FirstName</code> in a formula to get the current value of the element - what the user has entered as their first
            name.</Para>
        <Para><em>Note:</em> you will see that although you called the element "First Name", in the formula you write <code>FirstName</code> - just
            miss out the spaces.
        </Para>

        <MinorHeading>Fixed values in formulas</MinorHeading>
        <Para>
            As well as using the current value of an element, you can use fixed values, like <code>10</code> or <code>"Hello "</code> in the examples
            above.
        </Para>
        <Para>
        ...
        <Para>
            You enter a formula in <HelpLink id='element-properties'/>.
        ...
        <Para>
            This sets the Content property of a Text element, so that it will continuously update to show "Hello, " followed by whatver is typed into
            the Name input.
        </Para>
        ...
        <BulletList>
            <li><HelpLink id='calculation-formulas'>Calculations</HelpLink> - these work out values, but don't change anything</li>
            <li><HelpLink id='action-formulas'>Actions</HelpLink> - these make something happen, but they don't produce a value</li>
        </BulletList>
        ...
        <SubSection id='calculation-formulas'>
            ...
            <Para>These are like the basic maths expressions you learned at school, and they follow the same rules. Some things to look out
                for:</Para>
            <BulletList>
                <li>You use * instead of x to multiply values, to avoid confusion with something called "x"</li>
                <li>Multiplication and division are done before addition and subtraction, so you may need brackets (as you obviously remember from
                    school😇).
                </li>
            ...
            <BulletList>
                <li>maths calculations, such as adding up a list of numbers (<HelpLink id='Sum'/>), or finding the largest of a set of numbers
                    (<HelpLink id='Max'/>)
                </li>
                <li>working with text, such as getting the start of a piece of text (<HelpLink id='Left'/>)</li>
                <li>making decisions, such as choosing between two different calculations (<HelpLink id='If'/>)</li>
            </BulletList>
            <Para>
            ...
            <BulletList>
                ...
                <li>Left("bluebird", 4)<br/>
                    Get the first four characters of "bluebird",ie "blue"
                </li>
            ...
            <Para>Some functions are Action functions, which are used to update things or make something happen,
                and they can only be used in <HelpLink id='action-formulas'/>, not calculations.
                The <HelpLink id='functions-reference'/> section for a function tells you whether it is a Calculation function or an Action function -
                an Action function will say "Result type: action" at the top.</Para>

            <MinorHeading>Mix and Match</MinorHeading>
            <Para>A value in a formula does not have to be a fixed value or an element value. It can be the result of another formula. Here are some
                examples:
            </Para>
            <BulletList>
        ...
        <SubSection id='action-formulas'>
            ...
            <Para>There are some properties of elements that need an action formula instead of a calculation.
                The <HelpLink id='elements-reference'>reference section</HelpLink> for that element tells you which they are.</Para>
            ...
            <MinorHeading>Multiple actions</MinorHeading>
            <Para>Action formulas can contain multiple Action Functions. These are all done when the action is triggered,
                but they may happen in any order, so they must not depend on each other.</Para>
            <Para>You write each Action Function starting on a new line</Para>
            <Para>So if you wanted to extend the Action in the previous example to clear the data and log what it had done,
                it might look something like this:</Para>
            <TextField label="Action" variant='filled' size='small' multiline fullWidth
                       value={`Update(Orders, OrderId, OrderData)\nReset(OrderForm)\nLog("Added order: " + OrderId)`}/>
