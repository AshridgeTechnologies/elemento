...
const exposeBoundFunction = (objectName: string, functionName: string, object: any) => {
  const nameToExpose = objectName + '.' + functionName
  expose(nameToExpose, object[functionName].bind(object))
}

export const exposeFunctions = (objectName: string, serverObject: object) => {
  const functionNames = Object.getOwnPropertyNames(serverObject.constructor.prototype).filter(startsWithUppercase)
  functionNames.forEach(name => exposeBoundFunction(objectName, name, serverObject))
}